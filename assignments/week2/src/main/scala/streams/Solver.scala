...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    (for ((block, move) <- b.legalNeighbors) yield (block, move :: history)).to(LazyList)
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart filter (_._1 == goalBlock)
